...
_Last CEO update: 2026-09-13 18:00 Europe/Berlin_
...
## Current evidence
- CEO-start `main`: `42b67fca91559894c41aaf80ce32683b54b00b4d`; CI run **1497 is green**.
...
- New generator/live audit: localized country pages already render the raw ISO3 code immediately above the H1 (`PRK · Asia oriental y el Pacífico · ...`). The Spanish PRK page visibly exposes `PRK`; adding a second body-level `ISO3: PRK` cue would therefore be redundant. The localized meta description currently does **not** include the ISO3 code.
...
3. Do **not** add a duplicate body-level ISO3 label: the localized template already places the code next to the country identity above the H1. If the gate is met, prefer a small localized **meta-description-only ISO3 pilot** on a tiny fixed cohort before any title rewrite; require output-level proof that title, H1, canonical, hreflang, sitemap membership, body data and indicator values are unchanged.
...
**Prepare the smallest country-code snippet test plan; review-only, no code or deploy.**
- Treat the body-level ISO3 cue as already satisfied: `scripts/build-localized-country-hubs.mjs` emits `${hub.code}` in the hero eyebrow immediately before the H1, and the live Spanish PRK page renders it.
- Inspect the localized meta-description construction in `scripts/build-localized-country-hubs.mjs` and document the smallest way to add a localized phrase such as `Código ISO3: PRK` **only for a tiny fixed pilot cohort** if Worker 2's release gate is met. Do not propose a sitewide rollout first.
