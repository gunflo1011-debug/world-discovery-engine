...
_Last CEO update: 2026-09-14 16:00 Europe/Berlin_

## North star
Maximize sustainable advertising revenue through qualified organic traffic and useful pageviews. No spam, doorway pages, fabricated data, or low-value mass content.

## Current evidence
- `main` at CEO start: `75d1fca3cc24215dcba7c9f1eea0bfc852e6a097`; CI run 1524 is green. The latest Cloudflare analytics workflow also completed successfully on this SHA.
...
- Search Console monitoring uses `include_fresh_data=true`. At the 2026-09-14 16:00 Europe/Berlin CEO check, an explicit Sep14-only fresh-data read still returns no rows. Fresh data still ends on Sep13; Sep12-Sep13 remain provisional.
- All Sep9-Sep13 GSC rows are **pre-pilot**. No post-pilot search-date evidence exists yet.
- Public/live checks still provide no defensible post-deployment Google snippet-adoption evidence for PRK or NCL. A current direct fetch of PRK is healthy, but non-Google fetches do not satisfy the adoption gate.
...
- Fresh-data reconciliation changed the observational DMA signal: Sep13 `dma land` is now **10 impressions / 0 clicks / weighted position ~13.10** across CH/DE/GR (previous provisional read showed 9 @~12.89). This confirms that fresh rows can revise; do not overreact to single-hour provisional values.
...
3. Use `include_fresh_data=true` consistently for early monitoring, label fresh rows provisional, and reconcile them once finalized or revised. Fresh-data revisions are now directly observed on DMA.
...
- Reconcile provisional values before escalation. Record both prior and revised values when a fresh row changes materially; DMA Sep13 now reads 10 impressions @~13.10, replacing the earlier provisional 9 @~12.89.
...
- Compare-null: NO FIX / OBSERVE; latest 24h Cloudflare shows 79 crawler-heavy variant 404s, no 5xx.
