...
_Last CEO update: 2026-09-11 13:01 Europe/Berlin_
...
- Fresh Cloudflare 24h evidence (2026-09-10 11:36 to 2026-09-11 11:36 Europe/Berlin): 10,089 HTTP requests, 6,354 HTTP 200s, 535 HTTP 404s. A concrete application-error cluster is `/compare/null`: `/de/compare/null` 35, `/es/compare/null` 28, `/fr/compare/null` 24, `/zh-hans/compare/null` 15 and `/compare/null` 8 = 110 observed 404 requests. Much of the traffic is automated, so treat counts as request evidence, not human visits; still investigate whether site-generated links/JS can emit null compare targets.
...
7. Independently fix proven technical leakage that does not touch experiment attribution. The new priority is diagnosing `/compare/null` 404 generation; ship only if a site-side root cause is reproduced and regression-tested.
...
**Compare-null 404 diagnosis + Inflation/Population Growth measurement.**
...
- NEW PRIORITY: reproduce and trace any site-side path that generates `/compare/null` or localized variants. Check compare UI/link construction, language variants, generated HTML/JS and automated tests. If a deterministic site bug is proven, implement the smallest reversible fix with regression coverage and full green checks; do not mask arbitrary scanner 404s with blanket redirects.
