...
| WD-012 | P0 | CEO → Worker 4 | MOBILE_FIX_COMMITTED_WAITING_RERUN | After WD-016, execute exactly one full release gate: CI → Pages → release SHA → core/machine routes → Playwright mobile. If red, own full incident chain automatically. | WD-020 + WD-016 | `f445710`: CI, Pages deploy, commit-exact release identity and live machine contracts all passed for 182 countries. Mobile smoke exposed four stale/scaling assumptions: hard-coded 12-record count, ambiguous parent-link selector, AI URLs dropping the Pages base path, and mobile nav flex min-width overflow. All four are corrected together; local build and 43/43 Node tests are green. | Verify the changed mobile gate once at 360/390/430 px; if green, mark LIVE VERIFIED and close WD-020 → WD-016 → WD-012. |
...
- RELEASE INCIDENT: `6a74325` exposed missing fresh-checkout fixtures; `f445710` fixed the release order and passed CI/deploy/live contracts. Its browser gate then exposed one grouped expanded-corpus/mobile contract incident. Fixes now derive counts from live coverage, preserve the Pages base path, use an unambiguous semantic link selector, and constrain the mobile nav flex item.
