...
| WD-028D | P0 | Worker 4 | READY_WITH_INC_002 | Integrate A2–C and execute one full release gate with CI, Pages, exact SHA, core/machine/regional routes and 360/390/430 px smoke. | WD-028A2 + WD-028B + WD-028C | All three build handoffs are focused-green. W2 accidentally triggered partial releases; latest run `32642237393` deployed `a44122f` but failed the live contract because the workflow still requires country JSON schema 1.0 while A2 intentionally emits 1.1. Mobile was skipped. | Own INC-002: align the live contract to schema 1.1 plus region identity, add 7 regional Human+JSON/sitemap/build assertions, then intentionally trigger exactly one release. |
...
- RELEASE: production now serves W2 commit `a44122f` after an unintended intermediate Pages deployment. Exact SHA and deploy passed, but live contract run `32642237393` failed on stale schema 1.0 expectation and skipped mobile smoke. Therefore the current live state is NOT FULLY VERIFIED; W4 owns the corrective integrated release.
