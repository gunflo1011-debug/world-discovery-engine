...
| WD-028A2 | P0 | Worker 1 | READY | Propagate the accepted region contract into country-level machine JSON/index outputs and make their builders fail closed when `record.region.code/name` is missing; add focused regression tests. | WD-028A | The normalized source is accepted; country machine outputs do not yet expose region. | Hand focused green tests to W4. Expected contribution: consistent human/machine discovery and fewer downstream mapping defects. |
| WD-028B | P0 | Worker 2 | READY | Add a fast region filter and accessible region directory to the parent experience; preserve no-JS access and mobile containment. | WD-028A | Accepted contract exposes 7 official regions across all 182 records. | Focused UX tests, then hand off to W4. Expected contribution: faster country discovery and deeper engagement. |
| WD-028C | P0 | Worker 3 | READY | Generate exactly 7 substantial regional landing pages with original summaries, rankings scoped to the region, provenance, schema, canonical URLs and bidirectional links; no thin country/region combinations. | WD-028A | Counts: EAS 26, ECS 51, LCN 32, MEA 20, NAC 2, SAS 6, SSF 45. | Focused generator/discovery tests, then hand off to W4. Expected contribution: 7 high-intent organic/AI entry routes. |
| WD-028D | P0 | Worker 4 | RELEASE_OWNER_BLOCKED | Integrate A2–C and execute one full release gate with CI, Pages, exact SHA, core/machine/regional routes and 360/390/430 px smoke. | WD-028A2 + WD-028B + WD-028C | No work before build handoff. | LIVE VERIFIED or one owned incident chain. |
...
- Official regions in accepted normalized source: 7; live regional routes: 0
