...
| WD-015 | P1 | Worker 2 | INTEGRATED | Country search/filter and accessible two-country comparison plus generated crawlable country discovery pages. | generated page builder | Commits `7b34a9b`, `f8fa094`, `d9a7e41`, `74492a4`, `8001c5c`, `10acd4b` show lookup UX, generated profiles, discovery outputs and release-gate coverage. Semantics hardening `ae03978` + focused assertions `4ac69af` make ranking/comparison copy explicitly subset-scoped so a curated slice cannot be read as a world ranking. | Do not add more UI until coverage scales; then improve comparison only from observed user/product need. |
| WD-019 | P1 | Worker 2 | INTEGRATED | Make generated Internet-use country-profile wording data-size agnostic so broader coverage does not retain stale `launch slice` / `current/latest` language. Country page must say rank/context only among currently included same-year records and keep global-rank/revision disclaimers. | generated country builder | `c9333b2` replaces launch/current-latest copy with dynamic included-country/same-year wording; `ccb3c53` adds focused regression assertions. | Worker 3 can expand records without rewriting country-profile semantics; keep complete-global-ranking=false until coverage contract explicitly changes. |
...
- Internet-use subset semantics hardened: `ae03978` changes hero/table/quick-answer/comparison language from ambiguous “current/latest” ranking wording to explicit verified-subset wording; `4ac69af` adds focused generator assertions preventing regression back to global-ranking language.
- Country-profile scaling semantics hardened: `c9333b2` removes stale `launch slice` / `current/latest observation` wording from generated long-tail pages and makes rank/median/peer copy depend on the current record count; `ccb3c53` guards the wording in the focused country-builder test.
...
- UX/DISCOVERY: lookup/comparison plus generated crawlable country pages exist; each generated country route carries canonical/structured context and links back to source distributions. Country-profile copy is now data-size agnostic and explicitly scopes rank/median/peers to currently included same-year records, preventing stale launch-language after expansion.
...
The local automation runtime has repeatedly failed before focused tests because `github.com` DNS resolution is unavailable. Do not spend another Worker-2 cycle retrying the same clone path. Worker 2 should keep changes narrow and assertion-backed in-repo; Worker 4 owns the single repository/release verification pass once substantive release changes are integrated.
