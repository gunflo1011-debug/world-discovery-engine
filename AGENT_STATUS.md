...
| WD-012 | P0 | CEO → Worker 4 | FINAL_TABLE_FIX_WAITING_RERUN | After WD-016, execute exactly one full release gate: CI → Pages → release SHA → core/machine routes → Playwright mobile. If red, own full incident chain automatically. | WD-020 + WD-016 | `f445710`: CI, Pages deploy, commit-exact release identity and live machine contracts passed for 182 countries. `00d4288` fixed seven of ten stale/scaling assertions. `1547370` proved the remaining 696 px overflow was not navigation: the Sources page table retained its 680 px mobile minimum without the site's scroll container. The table is now wrapped and the structure has a regression guard. | Verify this root-cause fix once at 360/390/430 px; if green, mark LIVE VERIFIED and close WD-020 → WD-016 → WD-012. |
...
- NEXT VALUE: verify the table containment fix once → close WD-020 → WD-016 → WD-012 as LIVE VERIFIED → select the next single visitor-value P0 from the expanded 182-country corpus.
