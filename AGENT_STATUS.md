...
| WD-028D | P0 | Worker 4 | CORRECTIVE_RELEASE_TRIGGERED | Integrate A2–C and execute one full release gate with CI, Pages, exact SHA, core/machine/regional routes and 360/390/430 px smoke. | WD-028A2 + WD-028B + WD-028C | Run `32646143449` for `46ff764` proved deploy/SHA green but failed the live contract at BHR because the verifier checked `payload.region` while schema 1.1 defines `payload.entity.region`. `1d5fe9a` corrects that exact path; full regional gates remain unchanged. | Observe `1d5fe9a`. Green closes WD-028/INC-002; red continues from the exact failing step. |
...
- RELEASE: `46ff764` deployed exactly but its verifier failed because it read region metadata from `payload.region` instead of the schema-defined `payload.entity.region`. Corrective release `1d5fe9a` fixes that path and retains all 7 regional Human+JSON, sitemap/build and 360/390/430 px gates. `LIVE VERIFIED` remains pending.
