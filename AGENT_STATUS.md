...
| WD-028D | P0 | Worker 4 | RELEASE_TRIGGERED_WAITING_EVIDENCE | Integrate A2–C and execute one full release gate with CI, Pages, exact SHA, core/machine/regional routes and 360/390/430 px smoke. | WD-028A2 + WD-028B + WD-028C | `46ff764` aligns country JSON verification to schema 1.1 plus official region equality; verifies all 7 regional Human+JSON routes, sitemap uniqueness, build count and complete 182-country regional partition; adds regional browser checks at 360/390/430 px. | Observe exactly the W4-owned `46ff764` run. If green, close LIVE VERIFIED; if red, continue the owned INC-002 chain from the exact failing step. |
...
| INC-002 | Non-release-owner W2 commits triggered Pages; latest deploy reached exact SHA but live contract failed `country JSON identity mismatch BHR`, so browser smoke never ran | A2 intentionally bumped country machine JSON from schema 1.0 to 1.1 for official region identity, but the Pages live verifier remained hard-coded to 1.0. Release ownership guard was not followed on W2's intermediate commits. | `46ff764` updates schema/region equality and adds complete regional live/mobile regression coverage. | W4-owned release for `46ff764` is triggered; connector status is not yet observable. | FIX_COMMITTED_WAITING_VERIFICATION |
...
- RELEASE: corrective W4 release `46ff764` is triggered. Its gate now covers country schema 1.1/region equality, all 7 regional Human+JSON routes, sitemap/build consistency and regional mobile rendering at 360/390/430 px. `LIVE VERIFIED` remains pending until the exact run is observed green.
