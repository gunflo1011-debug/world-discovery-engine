...
| WD-012 | P0 | CEO → Worker 4 | FIX_COMMITTED_WAITING_RERUN | After WD-016, execute exactly one full release gate: CI → Pages → release SHA → core/machine routes → Playwright mobile. If red, own full incident chain automatically. | WD-020 + WD-016 | First expanded-corpus run `6a74325` failed in fresh checkout before build because `site/evidence/index.json` is generated and absent from Git. Root cause: changing `check` to tests-first removed required fixture generation. Durable order is build → tests → final build, preserving both fresh-checkout prerequisites and a clean deploy artifact. | Verify the changed release gate once; if green, confirm commit-exact live routes and close. |
...
- RELEASE INCIDENT: expanded-corpus commit `6a74325` failed because tests-first cannot run in a fresh checkout without generated evidence fixtures. The gate is corrected to build → tests → final build; the next run is the single verification of that changed gate.
