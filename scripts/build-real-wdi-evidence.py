    ...
    for row in rows:
        ...
        if country_code and value is not None:
            try:
                ...
                pass
    if ENTITY_CODE not in out:
        raise RuntimeError(f'{ENTITY_CODE} not found for {INDICATOR_CODE}')
    return out
...
def main():
    ...
    for rel in RELEASES:
        ...
        print(f"Loaded {INDICATOR_CODE}/{ENTITY_CODE} from {rel['vintage']} ({name})")

    first=snapshots[0]['data'][ENTITY_CODE]['value']
    latest=snapshots[1]['data'][ENTITY_CODE]['value']
    country=snapshots[1]['data'][ENTITY_CODE]['country']
    absolute=latest-first
    relrev=absolute/abs(first) if first else 0.0
    generated=datetime.now(timezone.utc).isoformat()
    evidence={
        "schemaVersion":"1.0","status":"REAL",
        "indicator":{"code":INDICATOR_CODE,"name":INDICATOR_NAME,"unit":UNIT,"methodologyVersion":METHODOLOGY_VERSION},
        "entity":{"code":ENTITY_CODE,"name":country,"entityType":"country"},
        ...
        "methodologyNote":"Population, total uses the World Bank population estimates/projections framework documented in its 2024 technical note. This evidence is deliberately limited to Germany, the same reference year, the same indicator identity and unit, and two releases within 2025. Regional aggregates are excluded because their membership/definition can change independently of the underlying country estimates.",
    ...
    with (OUT_DIR/'evidence.csv').open('w',newline='',encoding='utf-8') as f:
        ...
        for key in ('first','latest'):
            item=evidence[key]; w.writerow([ENTITY_CODE,country,INDICATOR_CODE,REFERENCE_YEAR,item['vintage'],item['value'],UNIT,item['sourceUrl']])
    ...
    html=f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{country} population revision — World Discovery Engine</title><meta name="description" content="Real World Bank WDI vintage comparison for {country} population in {REFERENCE_YEAR}, comparing the 28 Jan 2025 and 2 Jul 2025 database releases."><link rel="stylesheet" href="../../styles.css"></head><body><header class="topbar"><div class="wrap"><div class="brand">World Discovery Engine</div><nav class="nav"><a href="../../index.html">Home</a><a href="../index.html">Evidence</a><a href="../../methodology/index.html">Methodology</a></nav></div></header><main><section class="evidence-head"><div class="wrap"><span class="pill">REAL WDI EVIDENCE · COUNTRY-LEVEL · TWO ARCHIVED RELEASES</span><h1>{country}'s {REFERENCE_YEAR} population estimate was {direction} between two 2025 WDI releases.</h1><p class="muted">Generated directly from two official archived World Development Indicators releases. Regional aggregates are intentionally excluded from this first production evidence.</p></div></section><section class="section"><div class="wrap"><div class="facts"><div class="fact"><div class="label">28 Jan 2025 release</div><div class="value">{fmt_people(first)}</div><div class="muted">people</div></div><div class="fact"><div class="label">2 Jul 2025 release</div><div class="value">{fmt_people(latest)}</div><div class="muted">people</div></div><div class="fact"><div class="label">Relative revision</div><div class="value">{relative_text}</div><div class="muted">{absolute:+,.0f} people</div></div></div><h2 style="margin-top:34px">Revision history</h2><table class="table"><tr><th>Publication vintage</th><th>Reference year</th><th>Value</th><th>Source</th></tr><tr><td>{RELEASES[0]['vintage']}</td><td>{REFERENCE_YEAR}</td><td>{fmt_people(first)}</td><td><a href="{RELEASES[0]['url']}">World Bank archive</a></td></tr><tr><td>{RELEASES[1]['vintage']}</td><td>{REFERENCE_YEAR}</td><td>{fmt_people(latest)}</td><td><a href="{RELEASES[1]['url']}">World Bank archive</a></td></tr></table><h2 style="margin-top:34px">Comparability & provenance</h2><div class="sourcebox"><strong>Indicator:</strong> {INDICATOR_NAME} ({INDICATOR_CODE}) · <strong>Unit:</strong> {UNIT} · <strong>Entity:</strong> sovereign country ({ENTITY_CODE}) · <strong>Methodology gate:</strong> {METHODOLOGY_VERSION}. The comparison is restricted to the same indicator identity, unit, country, reference year and two WDI releases within 2025. Source license: CC BY 4.0.</div><p><a href="./evidence.json">JSON evidence</a> · <a href="./evidence.csv">CSV evidence</a></p></div></section></main><footer class="footer"><div class="wrap">World Discovery Engine · Real archived WDI evidence · Generated {generated}</div></footer></body></html>'''
    (OUT_DIR/'index.html').write_text(html,encoding='utf-8')
    print(json.dumps({"entity":country,"code":ENTITY_CODE,"first":first,"latest":latest,"relative_revision":relrev},indent=2))
