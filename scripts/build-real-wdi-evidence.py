...
def main():
    ...
    for code in ENTITY_CODES:
        ...
        if unchanged:
            description=f"Verified World Bank WDI vintage comparison showing that the published {REFERENCE_YEAR} population estimate for {country} was unchanged between January and July 2025."
        else:
            description=f"Verified World Bank WDI vintage comparison showing how the published {REFERENCE_YEAR} population estimate for {country} changed between January and July 2025."
        structured={
            "@context":"https://schema.org","@type":"Dataset",
            "name":f"{country} population revision 2025",
            "description":f"Verified comparison of the published {REFERENCE_YEAR} {country} population estimate across the 28 January 2025 and 2 July 2025 World Bank WDI archive releases.",
            "url":canonical,
            "identifier":f"{INDICATOR_CODE}-{code}-{REFERENCE_YEAR}-{RELEASES[0]['vintage']}-{RELEASES[1]['vintage']}",
            "creator":{"@type":"Organization","name":"World Discovery Engine"},
            "isBasedOn":[RELEASES[0]['url'],RELEASES[1]['url']],
            "spatialCoverage":{"@type":"Place","name":country},
            "temporalCoverage":f"{RELEASES[0]['vintage']}/{RELEASES[1]['vintage']}",
            "variableMeasured":{"@type":"PropertyValue","name":INDICATOR_NAME,"propertyID":INDICATOR_CODE,"unitText":UNIT},
            "distribution":[
                {"@type":"DataDownload","encodingFormat":"application/json","contentUrl":canonical+"evidence.json"},
                {"@type":"DataDownload","encodingFormat":"text/csv","contentUrl":canonical+"evidence.csv"}
            ]
        }
        structured_json=json.dumps(structured,separators=(',',':'))
        country_possessive=possessive(country)
        revision_percent="0.0000%" if unchanged else f"{relative*100:+.4f}%"
        revision_people="0 people" if unchanged else f"{absolute:+,.0f} people"
        if unchanged:
