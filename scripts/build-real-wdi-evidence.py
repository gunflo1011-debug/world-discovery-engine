...
def main():
    ...
    for code in ENTITY_CODES:
        ...
        evidence={"schemaVersion":"1.3","status":"REAL","finding":{"type":finding,"isRevision":absolute!=0},"indicator":{"code":INDICATOR_CODE,"name":INDICATOR_NAME,"unit":UNIT,"methodologyVersion":METHODOLOGY_VERSION},"entity":{"code":code,"name":country,"entityType":"country"},"referenceYear":REFERENCE_YEAR,
        ...
        provenance=f"Indicator: {INDICATOR_NAME} ({INDICATOR_CODE}) · Archived series name verified in both releases: {INDICATOR_NAME} · Unit: {UNIT} · Country: {code} · Reference year: {REFERENCE_YEAR} · Finding type: {finding} · Methodology gate: {METHODOLOGY_VERSION}. Regional aggregates are excluded. Source license: CC BY 4.0. Archive fingerprints: Jan {snapshots[0]['archive_sha256'][:12]}… · Jul {snapshots[1]['archive_sha256'][:12]}…; exact hashes, archived series names and archive members are included in JSON/CSV."
        ...
        manifest.append({"code":code,"country":country,"slug":page_slug,"finding":{"type":finding,"isRevision":absolute!=0},"absolute":absolute,"relative":relative,"first":{"vintage":RELEASES[0]['vintage'],"value":first},"latest":{"vintage":RELEASES[1]['vintage'],"value":latest}})
    (ROOT/'real-wdi-population-manifest.json').write_text(json.dumps({"schemaVersion":"1.3","generatedAt":generated,"indicator":{"code":INDICATOR_CODE,"name":INDICATOR_NAME,"referenceYear":REFERENCE_YEAR,"unit":UNIT},"archives":[{"vintage":s["vintage"],"url":s["url"],"archiveFile":s["archive_file"],"archiveSha256":s["archive_sha256"],"archiveBytes":s["archive_bytes"],"archivedIndicatorName":s["archived_indicator_name"]} for s in snapshots],"count":len(manifest),"items":manifest},indent=2)+"\n",encoding='utf-8')
