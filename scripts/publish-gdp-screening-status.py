...
def main():
    ...
    coverage=data.get("coverage",{}); require(coverage=={"requested":15,"rowsPresentInBothVintages":15,"missing":[]},"unexpected coverage")
    ...
    public={"schemaVersion":"1.5","screenedAtUtc":screened,"indicator":{"code":EXPECTED_CODE,"name":EXPECTED_NAME,"currentUnit":EXPECTED_UNIT,"referenceYear":EXPECTED_YEAR},"screeningStatus":"BLOCKED_METHODOLOGY_COMPARABILITY","publishableRevisionData":False,"coverage":coverage,"methodologyGate":{"archiveNamesConsistent":True,"archiveNamesMatchExpectedScreeningContract":True,"releaseSpecificBaseAndValuationVerified":False,"reason":gate.get("reason")},"provenance":{"dataset":provenance.get("dataset"),"datasetCatalogUrl":DATASET_CATALOG_URL,"archiveWarningUrl":provenance.get("archiveWarningUrl"),"currentMetadataUrl":provenance.get("currentMetadataUrl"),"releases":releases},"license":{"name":LICENSE_NAME,"url":LICENSE_URL,"datasetTermsUrl":DATASET_TERMS_URL,"attribution":"World Bank, World Development Indicators (WDI). World Discovery Engine derives screening/provenance status from archived WDI releases and does not imply World Bank endorsement."},"methodologyUrl":"https://gunflo1011-debug.github.io/world-discovery-engine/methodology/","screeningPageUrl":"https://gunflo1011-debug.github.io/world-discovery-engine/indicators/real-gdp/","provenancePageUrl":"https://gunflo1011-debug.github.io/world-discovery-engine/indicators/real-gdp/provenance.html","sourceCodeUrl":"https://github.com/gunflo1011-debug/world-discovery-engine/blob/main/scripts/screen-real-gdp.py","note":"Rows are present in both vintages, but they are not labeled methodologically comparable. No screened GDP values, revisions, rankings, or REAL CSV export are published while release-specific base-year and valuation comparability remains unresolved."}
