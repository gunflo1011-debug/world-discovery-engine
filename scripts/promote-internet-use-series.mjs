...
if (await exists(sitemapUrl)) {
  const sitemap = await readFile(sitemapUrl, 'utf8');
  await writeFile(sitemapUrl, removeLegacyHistoryRoutes(sitemap), 'utf8');
}

console.log(`Promoted ${promoted} internet-use country time series to primary country profiles and retired duplicate history routes from the sitemap.`);
