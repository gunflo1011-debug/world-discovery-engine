...
for (const item of indicators) {
  ...
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(item.name)} by country (${item.year}) — World Discovery</title><meta name="description" content="${esc(description)}"><link rel="canonical" href="https://worlddiscoverydata.com/data/${esc(item.slug)}/"><link rel="stylesheet" href="../../styles.css"><script type="application/ld+json">${JSON.stringify({ '@context':'https://schema.org', '@type':'Dataset', name:item.name, description, temporalCoverage:String(item.year), spatialCoverage:'World', creator:{'@type':'Organization',name:'World Bank'}, isBasedOn:`https://data.worldbank.org/indicator/${item.code}`, license:'https://creativecommons.org/licenses/by/4.0/' }).replaceAll('<','\\u003c')}</script></head><body><header class="topbar"><div class="wrap"><div class="brand"><a href="../../">World Discovery</a></div><nav class="nav"><a href="../../">Home</a><a href="../">Data</a><a href="../../indicators/">Indicators</a><a href="../../methodology/">About</a></nav></div></header><main><section class="hero"><div class="wrap"><div class="eyebrow">${esc(item.code)} · World Bank WDI · ${esc(item.year)}</div><h1>${esc(item.name)}</h1><p>Compare <strong>${records.length} countries</strong> in the newest same-year snapshot that meets World Discovery's coverage rule. No country is silently filled from a different year.</p><div class="hero-actions"><a href="#countries">Browse countries ↓</a>${dedicated ? `<a href="${dedicated}">Open detailed explorer →</a>` : ''}</div></div></section><section class="section"><div class="wrap"><div class="grid"><article class="card"><span class="pill">HIGHEST</span><div class="metric">${esc(formatValue(best.value, item.unit))}${esc(unitSuffix(item.unit))}</div><h3>${esc(best.country)}</h3></article><article class="card"><span class="pill">MIDDLE OBSERVATION</span><div class="metric">${esc(formatValue(middle.value, item.unit))}${esc(unitSuffix(item.unit))}</div><h3>${esc(middle.country)}</h3></article><article class="card"><span class="pill">LOWEST</span><div class="metric">${esc(formatValue(lowest.value, item.unit))}${esc(unitSuffix(item.unit))}</div><h3>${esc(lowest.country)}</h3></article></div></div></section><section class="section section-soft" id="countries"><div class="wrap"><div class="eyebrow">Country comparison</div><h2>${esc(item.name)} by country in ${esc(item.year)}</h2><p class="muted">Sorted from highest to lowest. Values shown here come directly from the verified same-year snapshot.</p><div style="overflow-x:auto"><table><thead><tr><th>Rank</th><th>Country</th><th>${esc(item.unit)}</th></tr></thead><tbody>${countryRows(records, item.unit)}</tbody></table></div></div></section><section class="section"><div class="wrap"><div class="eyebrow">Source & coverage</div><h2>What this dataset means</h2><div class="grid"><article class="card"><span class="pill">YEAR</span><div class="metric">${esc(item.year)}</div><h3>Same-year snapshot</h3><p>This is the newest year between ${esc(catalog?.selectionPolicy?.latestYear ?? item.year)} and ${esc(catalog?.selectionPolicy?.earliestYear ?? item.year)} with at least ${esc(catalog?.selectionPolicy?.minCountries ?? '')} eligible country observations.</p></article><article class="card"><span class="pill">COVERAGE</span><div class="metric">${records.length}</div><h3>Countries included</h3><p>Missing countries are not assigned zero and are not backfilled from older years.</p></article><article class="card"><span class="pill">HISTORY</span><div class="metric">${esc(history?.observations ?? '—')}</div><h3>Country-year observations</h3><p>${history?.years?.length ? `${esc(history.years.length)} years represented in the historical file.` : 'Historical data is available as a machine-readable file.'}</p></article></div><p><a href="../wdi/${esc(item.slug)}/data.json">Download current snapshot JSON →</a> · <a href="../wdi/${esc(item.slug)}/history.json">Download history JSON →</a> · <a href="https://data.worldbank.org/indicator/${encodeURIComponent(item.code)}">World Bank indicator source →</a></p></div></section></main><footer class="footer"><div class="wrap"><a href="../">← All official indicators</a> · World Discovery · Source-faithful global data.</div></footer></body></html>`;
  await writeFile(new URL('index.html', dir), html, 'utf8');
}

const cards = indicators.map((item) => `<article class="card"><span class="pill">${esc(item.code)} · ${esc(item.year ?? 'pending')}</span><div class="metric">${esc(item.countries)}</div><h3><a href="./${esc(item.slug)}/">${esc(item.name)}</a></h3><p>${item.year ? `Compare ${esc(item.countries)} countries in the latest well-covered ${esc(item.year)} snapshot.` : 'No snapshot currently meets the coverage threshold.'} Unit: ${esc(item.unit)}.</p><p><a href="./${esc(item.slug)}/">Explore countries →</a></p></article>`).join('');
const body = indicators.length ? `<section class="hero"><div class="wrap"><div class="eyebrow">Official global data · World Bank WDI</div><h1>Explore 30 global indicators.</h1><p>Choose a topic and compare countries using the newest well-covered same-year snapshot. Each page shows readable values first, with raw data and provenance available when you need them.</p><p><span class="pill">${indicators.length} INDICATORS · ${verified.length} VERIFIED SNAPSHOTS · OFFICIAL SOURCE</span></p></div></section><main><section class="section"><div class="wrap"><h2>Choose an indicator</h2><p class="muted">Population, economy, technology, health, energy and environment — all from the official World Development Indicators catalog.</p><div class="grid">${cards}</div></div></section></main>` : `<section class="hero"><div class="wrap"><h1>Data catalog refresh pending.</h1></div></section>`;
const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Explore global indicators — World Discovery</title><meta name="description" content="Browse 30 official World Bank WDI indicators and compare countries using the newest well-covered same-year data."><link rel="canonical" href="https://worlddiscoverydata.com/data/"><link rel="stylesheet" href="../styles.css"></head><body><header class="topbar"><div class="wrap"><div class="brand"><a href="../">World Discovery</a></div><nav class="nav"><a href="../">Home</a><a href="./" aria-current="page">Data</a><a href="../indicators/">Indicators</a><a href="../methodology/">About</a></nav></div></header>${body}<footer class="footer"><div class="wrap">World Discovery · Official global data with explicit years and sources.</div></footer></body></html>`;
