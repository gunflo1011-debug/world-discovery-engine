...
function countryLink(record) {
  return `<a href="${countryHref(record.code)}"><strong>${esc(record.country)}</strong></a>`;
}
function countryRows(records, unit) {
  return records.map((r, i) => `<tr><td>${i + 1}</td><td>${countryLink(r)}<br><span class="muted">${esc(r.code)} · ${esc(r.region?.name ?? '')}</span></td><td>${esc(formatValue(r.value, unit))}${esc(unitSuffix(unit))}</td></tr>`).join('');
}
