...
function localizedData(locale, cfg) {
  ...
  const cards = (catalog.indicators ?? []).map((item) => `<article class="localized-card"><span class="pill">${esc(item.code)} · ${esc(item.year ?? '')}</span><h2>${esc(item.name)}</h2><p>${esc(item.unit ?? '')}</p><a href="/data/${esc(item.slug)}/?lang=${encodeURIComponent(locale)}">${esc(cfg.openIndicator)} →</a></article>`).join('');
