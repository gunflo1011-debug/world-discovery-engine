...
function localizedData(locale, cfg) {
  ...
  const cards = (catalog.indicators ?? []).map((item) => {
    ...
    return `<article class="localized-card"><span class="pill">${esc(item.code)} · ${esc(item.year ?? '')}</span><h2>${esc(translatedIndicatorName(cfg, item))}</h2><p>${esc(item.unit ?? '')}</p><a href="${esc(href)}">${esc(action)} →</a></article>`;
  }).join('');
...
function localizedIndicator(locale, cfg, item, snapshot) {
  ...
  const rows = records.map((record, index) => `<tr data-country="${esc(String(record.country).toLocaleLowerCase(locale))}"><td>${index + 1}</td><td><strong>${esc(record.country)}</strong><br><span class="muted">${esc(record.code)} · ${esc(record.region?.name ?? '')}</span></td><td>${esc(formatLocalizedValue(record.value, item.unit, locale))}</td></tr>`).join('');
  ...
  return `<!doctype html><html lang="${esc(cfg.htmlLang)}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(name)} (${esc(item.year)}) — World Discovery</title><meta name="description" content="${esc(description)}"><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="${esc(canonical)}">${hreflangLinks(`data/${item.slug}/`)}<link rel="stylesheet" href="/styles.css">${sharedStyle}</head><body><header class="topbar"><div class="wrap"><a class="brand" href="${esc(localeUrl(locale))}">World Discovery</a></div></header>${languageSwitcher(locale, `data/${item.slug}/`)}<main><section class="hero"><div class="wrap"><div class="eyebrow">${esc(item.code)} · World Bank WDI · ${esc(item.year)}</div><h1>${esc(name)}</h1><p>${esc(records.length)} ${esc(cfg.coverage)}.</p><p class="muted">${esc(cfg.officialLabel)}: ${esc(item.name)} · ${esc(item.unit)}</p><p class="muted">${esc(cfg.sourceNote)}</p></div></section><section class="section"><div class="wrap"><label for="country-search"><strong>${esc(cfg.searchCountries)}</strong></label><br><input id="country-search" class="localized-search" data-country-search type="search" placeholder="${esc(cfg.searchPlaceholder)}" autocomplete="off"><div class="localized-table-wrap"><table class="localized-table"><thead><tr><th>${esc(cfg.rank)}</th><th>${esc(cfg.country)}</th><th>${esc(cfg.value)} · ${esc(item.unit)}</th></tr></thead><tbody>${rows}</tbody></table></div><p><a href="${esc(localeUrl(locale, 'data/'))}">← ${esc(cfg.backToData)}</a></p></div></section></main><footer class="footer"><div class="wrap">World Discovery · ${esc(cfg.sourceNote)}</div></footer>${script}</body></html>`;
}
