...
for (const fact of facts) {
  ...
  if (count > 2) throw new Error(`Daily limit violated: more than two fun facts for ${fact.date}`);
  dailyCounts.set(fact.date, count);
}

const ui = {
  en: {home:'Home',explore:'Explore',data:'Data',trends:'Trends',funFacts:'Fun Facts',countries:'Countries',compare:'Compare',about:'About',nav:'Main navigation',detailEyebrow:'Fun Fact',yes:'Yes, really.',why:'Why this is so weird',what:'What is actually happening?',deeper:'The detail that makes it even better',sources:'Sources',published:'Published',updated:'Updated',checked:'Claims checked against the sources above.',more:'← More Fun Facts',hubEyebrow:'Up to two genuinely weird facts a day',hubTitle:'Fun Facts that make you say “wait… what?”',hubIntro:'Curious, absurd or surprising facts — selected by hand, explained with context and checked against serious sources. No rankings pretending to be trivia.',read:'Read the checked explanation →',pageTitle:'Fun Facts that are actually fun · World Discovery',pageDescription:'Up to two surprising, source-checked fun facts a day — with the context behind each claim.',collectionName:'World Discovery Fun Facts',footer:'World Discovery · Curious facts, checked against serious sources.',socialAlt:'World Discovery — source-checked global data and discoveries'},
  de: {home:'Startseite',explore:'Entdecken',data:'Daten',trends:'Trends',funFacts:'Fun Facts',countries:'Länder',compare:'Vergleichen',about:'Über uns',nav:'Hauptnavigation',detailEyebrow:'Fun Fact',yes:'Ja, wirklich.',why:'Warum das so erstaunlich ist',what:'Was passiert dabei eigentlich?',deeper:'Das Detail, das den Fakt noch besser macht',sources:'Quellen',published:'Veröffentlicht',updated:'Aktualisiert',checked:'Die Aussagen wurden anhand der oben genannten Quellen geprüft.',more:'← Mehr Fun Facts',hubEyebrow:'Bis zu zwei wirklich erstaunliche Fakten pro Tag',hubTitle:'Fun Facts, bei denen man zweimal hinschaut',hubIntro:'Kuriose, absurde oder überraschende Fakten — von Hand ausgewählt, mit Kontext erklärt und mit seriösen Quellen geprüft. Keine Rankings, die nur als Trivia verkleidet sind.',read:'Geprüfte Erklärung lesen →',pageTitle:'Fun Facts, die wirklich Spaß machen · World Discovery',pageDescription:'Bis zu zwei erstaunliche, mit Quellen geprüfte Fun Facts pro Tag — inklusive Hintergrund und Einordnung.',collectionName:'World Discovery Fun Facts',footer:'World Discovery · Kuriose Fakten, geprüft mit seriösen Quellen.',socialAlt:'World Discovery — geprüfte globale Daten und Entdeckungen'},
  es: {home:'Inicio',explore:'Explorar',data:'Datos',trends:'Tendencias',funFacts:'Datos curiosos',countries:'Países',compare:'Comparar',about:'Acerca de',nav:'Navegación principal',detailEyebrow:'Dato curioso',yes:'Sí, de verdad.',why:'Por qué resulta tan sorprendente',what:'¿Qué ocurre realmente?',deeper:'El detalle que lo hace aún más interesante',sources:'Fuentes',published:'Publicado',updated:'Actualizado',checked:'Las afirmaciones se comprobaron con las fuentes anteriores.',more:'← Más datos curiosos',hubEyebrow:'Hasta dos datos realmente sorprendentes al día',hubTitle:'Datos curiosos que te hacen decir «espera… ¿qué?»',hubIntro:'Datos curiosos, absurdos o sorprendentes, seleccionados a mano, explicados con contexto y comprobados con fuentes serias. Sin rankings disfrazados de curiosidades.',read:'Leer la explicación verificada →',pageTitle:'Datos curiosos que de verdad sorprenden · World Discovery',pageDescription:'Hasta dos datos curiosos sorprendentes y verificados al día, con contexto y explicación.',collectionName:'Datos curiosos de World Discovery',footer:'World Discovery · Datos curiosos comprobados con fuentes serias.',socialAlt:'World Discovery — datos globales y descubrimientos verificados'},
  fr: {home:'Accueil',explore:'Explorer',data:'Données',trends:'Tendances',funFacts:'Faits insolites',countries:'Pays',compare:'Comparer',about:'À propos',nav:'Navigation principale',detailEyebrow:'Fait insolite',yes:'Oui, vraiment.',why:'Pourquoi c’est si étonnant',what:'Que se passe-t-il réellement ?',deeper:'Le détail qui rend ce fait encore plus étonnant',sources:'Sources',published:'Publié',updated:'Mis à jour',checked:'Les affirmations ont été vérifiées à partir des sources ci-dessus.',more:'← Plus de faits insolites',hubEyebrow:'Jusqu’à deux faits vraiment étonnants par jour',hubTitle:'Des faits insolites qui font dire « attends… quoi ? »',hubIntro:'Des faits curieux, absurdes ou surprenants, sélectionnés à la main, expliqués avec contexte et vérifiés auprès de sources sérieuses. Pas de classements déguisés en anecdotes.',read:'Lire l’explication vérifiée →',pageTitle:'Des faits insolites vraiment étonnants · World Discovery',pageDescription:'Jusqu’à deux faits étonnants vérifiés par jour, avec contexte et explications.',collectionName:'Faits insolites de World Discovery',footer:'World Discovery · Des faits curieux vérifiés auprès de sources sérieuses.',socialAlt:'World Discovery — données mondiales et découvertes vérifiées'},
  'zh-Hans': {home:'首页',explore:'探索',data:'数据',trends:'趋势',funFacts:'趣味事实',countries:'国家',compare:'比较',about:'关于',nav:'主导航',detailEyebrow:'趣味事实',yes:'没错，是真的。',why:'为什么这件事如此惊人',what:'它到底是怎么发生的？',deeper:'让这个事实更精彩的细节',sources:'来源',published:'发布于',updated:'更新于',checked:'以上说法已根据上述来源核实。',more:'← 查看更多趣味事实',hubEyebrow:'每天最多两个真正令人惊讶的事实',hubTitle:'让你忍不住说“等等……什么？”的趣味事实',hubIntro:'人工挑选有趣、离奇或令人惊讶的事实，补充背景解释，并用可靠来源核实。这里不会把普通排名包装成冷知识。',read:'阅读已核实的解释 →',pageTitle:'真正有趣的趣味事实 · World Discovery',pageDescription:'每天最多两个令人惊讶并经过可靠来源核实的趣味事实，附带背景和解释。',collectionName:'World Discovery 趣味事实',footer:'World Discovery · 用可靠来源核实有趣事实。',socialAlt:'World Discovery — 经核实的全球数据与发现'}
};
...
for (const locale of locales) {
  ...
  for (const f of facts) {
    ...
    const body=`<section class="hero hero-compact"><div class="wrap"><div class="fact-emoji" aria-hidden="true">${f.emoji}</div><div class="eyebrow">${esc(t.detailEyebrow)} · ${esc(f.date)}</div><h1>${esc(lf.title)}</h1><p class="fact-answer"><strong>${esc(t.yes)}</strong> ${esc(lf.dek)}</p></div></section><main><section class="section"><div class="wrap"><h2>${esc(t.why)}</h2><p>${esc(lf.why)}</p><h2>${esc(t.what)}</h2><p>${esc(lf.context)}</p>${extra}<h2>${esc(t.sources)}</h2>${sources}<p class="muted fact-meta">${esc(t.published)} ${esc(f.date)}${modified!==f.date?` · ${esc(t.updated)} ${esc(modified)}`:''} · ${esc(t.checked)}</p><p><a href="${prefix(locale)}/fun-facts/">${esc(t.more)}</a></p></div></section></main>`;
...
console.log(`Built ${facts.length} curated fun fact(s) across ${locales.length} locale(s); maximum two per calendar day.`);
