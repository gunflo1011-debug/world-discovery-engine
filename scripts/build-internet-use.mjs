...
function makeHtml(data) {
  ...
  const germanyAnswer = germany ? `Within this verified ${data.records.length}-country snapshot, Germany ranks ${germany.rank}${germany.rank === 1 ? 'st' : germany.rank === 2 ? 'nd' : germany.rank === 3 ? 'rd' : 'th'} at ${germany.value}%.` : 'Germany is not included in this verified snapshot.';
  ...
  const browserScript = `(()=>{const tools=document.getElementById('internet-tools');const table=document.getElementById('internet-table');if(!tools||!table)return;const rows=[...table.tBodies[0].rows];const search=document.getElementById('country-search');const region=document.getElementById('region-filter');const clear=document.getElementById('country-clear');const status=document.getElementById('country-status');const a=document.getElementById('compare-a');const b=document.getElementById('compare-b');const result=document.getElementById('compare-result');const share=document.getElementById('share-view');const byCode=new Map(rows.map(row=>[row.dataset.code,{country:row.cells[1].textContent.trim(),value:Number(row.dataset.value),rank:Number(row.dataset.rank),region:[...region.options].find(option=>option.value===row.dataset.region)?.textContent||row.dataset.region}]));const compareEntries=[...byCode.entries()].sort(([,left],[,right])=>left.country.localeCompare(right.country));for(const select of [a,b])for(const [code,entry] of compareEntries){const option=document.createElement("option");option.value=code.toUpperCase();option.textContent=entry.country+" — "+entry.value+"%";select.append(option);}tools.hidden=false;const syncUrl=()=>{const url=new URL(location.href);const query=search.value.trim();if(query)url.searchParams.set('q',query);else url.searchParams.delete('q');if(region.value)url.searchParams.set('region',region.value);else url.searchParams.delete('region');if(a.value&&b.value)url.searchParams.set('compare',a.value+','+b.value);else url.searchParams.delete('compare');const relative=url.pathname+url.search+url.hash;history.replaceState(null,'',relative);share.href=relative;};const applyFilter=()=>{const query=search.value.trim().toLowerCase();const regionCode=region.value;let visible=0;for(const row of rows){const matchesQuery=!query||row.dataset.country.includes(query)||row.dataset.code.includes(query);const matchesRegion=!regionCode||row.dataset.region===regionCode;const match=matchesQuery&&matchesRegion;row.hidden=!match;if(match)visible++;}const scope=regionCode?region.options[region.selectedIndex].textContent:'all regions';status.textContent=visible?('Showing '+visible+' '+(visible===1?'country':'countries')+' in '+scope+(query?' matching “'+search.value.trim()+'”':'')+'.'):('No countries match these filters.');};const compare=()=>{const left=byCode.get(a.value.toLowerCase());const right=byCode.get(b.value.toLowerCase());if(!left||!right){result.hidden=true;result.textContent="";return;}if(a.value===b.value){result.hidden=false;result.textContent="Within this verified ${data.observationYear} snapshot, "+left.country+" ranks #"+left.rank+" of "+rows.length+" in "+left.region+" at "+left.value+"%. Choose a different second country to compare.";return;}const gap=Math.abs(left.value-right.value);const higher=left.value===right.value?null:(left.value>right.value?left:right);const context=left.country+" ranks #"+left.rank+" of "+rows.length+" in "+left.region+" at "+left.value+"%; "+right.country+" ranks #"+right.rank+" of "+rows.length+" in "+right.region+" at "+right.value+"%.";result.hidden=false;result.textContent=higher?(context+" "+higher.country+" is higher by "+(Number.isInteger(gap)?String(gap):gap.toFixed(1))+" percentage points."):(context+" They are tied.");};const refresh=()=>{applyFilter();compare();syncUrl();};const params=new URLSearchParams(location.search);const initialQuery=params.get('q');if(initialQuery)search.value=initialQuery.slice(0,80);const initialRegion=(params.get('region')||'').toLowerCase();if([...region.options].some(option=>option.value===initialRegion))region.value=initialRegion;const initialCompare=(params.get('compare')||'').toUpperCase().split(',');if(byCode.has((initialCompare[0]||'').toLowerCase()))a.value=initialCompare[0];if(byCode.has((initialCompare[1]||'').toLowerCase()))b.value=initialCompare[1];search.addEventListener('input',refresh);region.addEventListener('change',refresh);clear.addEventListener('click',()=>{search.value='';region.value='';refresh();search.focus();});a.addEventListener('change',refresh);b.addEventListener('change',refresh);refresh();})();`;
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}</title><meta name="description" content="${esc(description)}"><link rel="canonical" href="${canonical}"><link rel="stylesheet" href="../../styles.css"><script type="application/ld+json">${JSON.stringify(dataset)}</script></head><body><header class="topbar"><div class="wrap"><div class="brand">World Discovery Engine</div><nav class="nav" aria-label="Primary"><a href="../../index.html">Home</a><a href="../../evidence/index.html">Evidence</a><a href="../index.html" aria-current="page">Indicators</a><a href="../../explore/index.html">Explore</a><a href="../../methodology/index.html">Methodology</a><a href="../../sources/index.html">Sources</a></nav></div></header><main><section class="hero hero-compact"><div class="wrap"><div class="eyebrow">Internet use · verified 2024 snapshot</div><h1>What share of people used the internet in ${data.observationYear}?</h1><p>Across ${data.records.length} verified non-aggregate countries with ${data.observationYear} observations, internet use ranges from ${min}% to ${max}% of the population. ${esc(leaderText)} ${leaders.length === 1 ? 'is' : 'are'} highest in this same-year snapshot at ${max}%${germany ? `; Germany is ${germany.value}%` : ''}.</p><span class="pill">${data.observationYear} OBSERVATIONS · ${data.records.length} VERIFIED COUNTRIES · ITU VIA WDI</span></div></section><section class="section"><div class="wrap"><h2>Internet use by country — ${data.observationYear} observations</h2><p>This table compares all ${data.records.length} non-aggregate countries returned by the verified ${data.observationYear} source snapshot. Countries without a ${data.observationYear} observation are omitted rather than backfilled from older years.</p><div class="internet-tools" id="internet-tools" hidden><div class="internet-lookup"><label class="searchbox" for="country-search">Find a country<input id="country-search" type="search" inputmode="search" autocomplete="off" placeholder="Type Germany, DEU…"></label><button type="button" id="country-clear">Clear filters</button></div><label class="internet-region-filter" for="region-filter">Region<select id="region-filter"><option value="">All regions (${data.records.length})</option>${regionOptions}</select></label><div class="internet-compare"><label>Compare country<select id="compare-a"><option value="">Choose a country</option></select></label><label>With<select id="compare-b"><option value="">Choose a country</option></select></label></div><p id="country-status" class="internet-tool-status" aria-live="polite">Showing all ${data.records.length} verified countries.</p><p><a id="share-view" href="./">Share this filtered view →</a></p><p id="compare-result" class="internet-compare-result" aria-live="polite" hidden></p></div><p><a id="country-table-jump" href="#internet-table">Skip directly to country comparison table →</a></p><nav class="region-directory" aria-labelledby="region-directory-heading"><h3 id="region-directory-heading">Browse all countries by region</h3><p>Open a region to reach every included country profile. This directory works without JavaScript.</p><div class="region-directory-grid">${regionDirectory}</div></nav><div class="table-wrap"><table class="table internet-table" id="internet-table" tabindex="-1"><caption>${esc(data.indicator.name)}, ${data.observationYear} verified same-year snapshot</caption><thead><tr><th scope="col">Rank in snapshot</th><th scope="col">Country</th><th scope="col">Internet users</th><th scope="col">Observation year</th></tr></thead><tbody>${tableRows}</tbody></table></div><p><a href="./data.json">Download JSON →</a> · <a href="./data.csv">Download CSV →</a></p></div></section><section class="section section-soft"><div class="wrap"><h2>Quick answers from this verified snapshot</h2><div class="grid"><article class="card"><h3>Which included countries are highest?</h3><p>${esc(leaderText)} ${leaders.length === 1 ? 'has' : 'have'} the highest observed share among these ${data.records.length} countries at ${max}%.</p></article><article class="card"><h3>Where does Germany sit here?</h3><p>${esc(germanyAnswer)}</p></article><article class="card"><h3>How wide is the observed range?</h3><p>${esc(rangeAnswer)}</p></article></div></div></section><section class="section"><div class="wrap"><h2>Source and provenance</h2><p><strong>Indicator:</strong> ${esc(data.indicator.code)} — ${esc(data.indicator.name)}.</p><p><strong>Source:</strong> ${esc(data.source.publisher)}, ${esc(data.source.dataset)}, surfaced via ${esc(data.source.surface)}.</p><p><strong>Retrieval:</strong> ${esc(retrieved)}.</p><p><a href="${esc(data.retrievalUrl)}">World Bank indicator page →</a> · <a href="${esc(data.source.metadataUrl)}">WDI metadata →</a></p></div></section></main><footer class="footer"><div class="wrap">World Discovery Engine · Same-year comparisons with source-backed provenance.</div></footer><script>${browserScript}</script></body></html>`;
}

const data = JSON.parse(await readFile(jsonUrl, 'utf8'));
validate(data);
await writeFile(csvUrl, makeCsv(data));
await writeFile(htmlUrl, makeHtml(data));
console.log(`Built internet-use product for ${data.observationYear}: ${data.records.length} records.`);
