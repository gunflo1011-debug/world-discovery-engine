...
function makeHtml(data) {
  ...
  const regionDirectory = regions.map((region) => `<details class="region-directory-item" id="region-${esc(region.code.toLowerCase())}"><summary><span>${esc(region.name)}</span><span>${region.records.length} countries</span></summary><ul>${region.records.map((record) => `<li><a href="./country/${esc(record.code.toLowerCase())}/">${esc(record.country)}</a></li>`).join('')}</ul></details>`).join('');
