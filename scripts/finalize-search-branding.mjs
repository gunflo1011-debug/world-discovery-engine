...
const links = `<link rel="icon" href="${origin}/favicon.svg" type="image/svg+xml" ${marker}="primary">`;
...
function finalize(html, file) {
  ...
  if (!next.includes(`href="${origin}/favicon.svg"`)) {
    throw new Error(`Search-brand favicon link missing after finalization: ${path.relative(root, file)}`);
  }
  return next;
}
