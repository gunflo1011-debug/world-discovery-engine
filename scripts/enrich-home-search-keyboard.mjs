...
const activeStyle = '<style>.search-suggestion[aria-selected="true"]{background:#eff6ff}</style>';

if (html.includes(newKeys)) {
  if (!html.includes(activeStyle)) html = html.replace('</head>', `${activeStyle}</head>`);
