...
function relatedRecords(data, record, limit = 4) {
  const latest = record.observations.at(-1);
  return data.records
    .filter(candidate => candidate.code !== record.code && candidate.region === record.region && Array.isArray(candidate.observations) && candidate.observations.length)
    .map(candidate => ({candidate, distance: Math.abs(candidate.observations.at(-1).value - latest.value)}))
    .sort((a, b) => a.distance - b.distance || a.candidate.country.localeCompare(b.candidate.country))
    .slice(0, limit)
