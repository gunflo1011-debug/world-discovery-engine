...
function chart(record) {
  ...
  return `<svg viewBox="0 0 ${width} ${height}" role="img" aria-labelledby="chart-title chart-desc" style="width:100%;height:auto;display:block;max-width:${width}px;margin:20px 0"><title id="chart-title">${esc(record.country)} internet use over time</title><desc id="chart-desc">Verified World Bank WDI observations for ${esc(record.country)} from ${minYear} to ${maxYear}. Missing years are not interpolated.</desc>${grid}<line x1="${left}" y1="${height-bottom}" x2="${width-right}" y2="${height-bottom}" stroke="currentColor" opacity="0.45"/><polyline points="${polyline}" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/><text x="${left}" y="${height-8}" text-anchor="start" font-size="12">${minYear}</text><text x="${width-right}" y="${height-8}" text-anchor="end" font-size="12">${maxYear}</text></svg>`;
}
