...
function relatedRecords(data, record, limit = 4) {
  const latest = record.observations.at(-1);
  return data.records
    .filter(candidate => candidate.code !== record.code && candidate.region?.code === record.region?.code && Array.isArray(candidate.observations) && candidate.observations.length)
...
function page(data, record) {
  ...
  const relatedSection = relatedCards ? `<section class="section section-soft"><div class="wrap"><h2>Compare ${esc(record.country)} with similar regional rates</h2><p>These ${esc(record.region.name)} countries have the closest latest observed internet-penetration rates in the same verified history dataset. The links are based on data proximity, not a claim of social or economic similarity.</p><div class="grid">${relatedCards}</div><p><a href="../../../region/${record.region.code.toLowerCase()}/">Explore the regional comparison →</a></p></div></section>` : '';
