...
for (const item of indicators) {
  ...
  const intentBridge = item.code === 'IT.NET.USER.ZS'
    ? `<section class="section" data-broad-intent-explainer="internet-use"><div class="wrap"><div class="eyebrow">Measure guide</div><h2>Internet penetration explained</h2><p>On this page, internet penetration means the share of individuals who used the internet, using World Bank World Development Indicators code <strong>${esc(item.code)}</strong>. A value of 72.4 means 72.4% of the population used the internet in the reported year.</p><p>This percentage is useful for comparing internet use or internet penetration by country. It is <strong>not</strong> the absolute number of internet users, and it does not measure household access, connection speed, fixed-broadband subscriptions or mobile subscriptions. Absolute user counts require a separate population estimate.</p></div></section>`
    : '';
  ...
  html = html.replace(anchor, `${intentBridge}${block}${anchor}`);
