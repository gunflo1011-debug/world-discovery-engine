...
for (const record of data.records) {
  ...
  const intentSection = `<section class="section section-soft" ${marker}><div class="wrap"><h2>What does “internet penetration rate” mean here?</h2><p>On this page, “internet penetration rate” and “internet users” refer to the percentage of individuals who used the internet, matching World Development Indicators code <strong>${esc(data.indicator.code)}</strong>. It is a population-use measure; it is not the same as fixed-broadband subscriptions, mobile subscriptions, connection speed or household access.</p><p><a href="/data/internet-use/">Compare internet users by country and year →</a> · <a href="../../">Explore the ${year} internet-use ranking →</a></p><p>The observation shown here is <strong>${year}</strong>. The retrieval date is kept separately in the provenance section so a newer download date is never presented as a newer observation year.</p></div></section>`;
