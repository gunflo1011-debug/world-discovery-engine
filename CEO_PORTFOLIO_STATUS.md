...
|---|---|---|---|
| World Discovery | Public product; 182 official 2024 country profiles and 7 regions are live. All 189 country/region pages expose source-faithful JSON-LD, and the 216-route graph now has 3,784 validated internal links with zero orphans at live release `4867b6b`; CI `32703208355` and Pages `32703208361` passed. | Search impressions, clicks, queries, indexed pages, organic visits, returning users, AI referrals, revenue, costs, and net profit are **UNKNOWN / NOT INSTRUMENTED**. | Strongest scalable/passive asset; Search Console is only the acquisition-measurement blocker while product, discovery and performance work continue. |
...
|---|---|---|---|---|
| World Discovery Solo Builder | **Measure and improve the real mobile performance of the existing country-parent experience.** Keep Search Console as a separate PARTIAL_BLOCKER. | Record reproducible baseline page weight, request count and render-critical assets for representative 360/390/430 px routes; implement exactly one evidence-backed optimization with no functionality/content loss; regression tests, CI and Pages green; record before/after numbers and exact live SHA. | `google*.html` blocks only Search Console verification/sitemap submission. | Improves acquisition, usability and crawl efficiency on the already-live corpus without speculative feature expansion. |
| Things App Worker | **Deliver the local smartphone-intake data contract and validator as the first slice of the approved UX.** Keep PR #3 unmerged and do not touch hosted wiring. | Typed local model plus unit tests for model, storage, condition/defects, optional battery health, activation-lock removal readiness, lawful-ownership confirmation, price floor and coarse area. Reject missing/unsafe values; prohibit credentials, full IMEI/serial and full address. TypeScript, tests, export/build and secret scan green. Commit or document an exact local blocker this cycle. | Hosted test-user secrets block only the later hosted Auth/RLS smoke. | Creates a small verifiable foundation for the owner intake while avoiding another oversized delivery miss. |
| Worker 4 | **Perform a read-only access-path and RLS audit for the seven private-schema Things tables.** Do not change schema, hosted data, PR #3 or app UX. | Map every client/RPC path to table/function; verify schema exposure and grants separately from row policies; check RLS defense-in-depth, owner predicates, UPDATE `USING` + `WITH CHECK`, view security-invoker behavior, function execute grants/`SECURITY DEFINER`, metadata-based authorization and cross-owner/BOLA risks. Rebuild/run existing local pgTAP gates where possible and return PASS/FINDINGS/BLOCKED with exact evidence and prioritized remediation. | No hosted credentials required for repository/local audit; current Supabase docs/changelog must be checked before conclusions. | Reduces privacy and breach risk before any Karlsruhe participant data exists, without overlapping the Things feature slice. |
