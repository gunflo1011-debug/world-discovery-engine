...
| Worker 4 | **US-only POD pre-mortem, read-only; preserve the Render handoff.** Determine whether a US organic test can plausibly beat the existing assets before any new build or listing work. | Document current official US production/shipping inputs, Etsy fee/tax assumptions, returns/compliance obligations, three demand/competition signals, contribution by retained variant, operational workload, and `GO/HOLD/KILL`. Require modeled contribution >= EUR 8 (or USD equivalent) without mandatory ads; cite dated sources. No product mutation, publish, order, payment or ad. If `RENDER_DEPLOY_HOOK_URL` appears, pause research and execute the existing exact 0.10.2/SHA readback once. | Public/current evidence may be used; exact authenticated readback remains externally blocked by the Render hook. | Uses otherwise idle capacity to reject or validate the only remaining near-term POD market without reopening EEA risk. |
