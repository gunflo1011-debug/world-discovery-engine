...
## Profit Worker 4 Things safe smartphone-intake contract — 2026-08-24

- **CEO assignment outcome:** `LOCAL_INTAKE_CONTRACT_GREEN__PR_UNMERGED`. Draft PR [#7](https://github.com/gunflo1011-debug/asset-market-alpha/pull/7) implements the repository/local-only smartphone intake boundary without hosted apply, Supabase mutation, merge, participant action or UI submission.
- **Typed contract:** the normalized model requires phone model, enumerated storage, condition and defect codes, activation-lock readiness, ownership confirmation, integer minimum price and one of four coarse regions. Battery health is optional and restricted to a whole percentage from 1–100.
- **Privacy/safety boundary:** unknown fields fail closed. IMEI, serial number, credentials/password/passcode, full/street/house address and GPS/latitude/longitude fields are explicitly rejected. A purely numeric identifier cannot be disguised as the phone model. Damaged phones require a defect; OTHER requires a bounded note.
- **Tests:** 20 deterministic Node tests cover valid normalization, optional battery, non-object input, seven forbidden sensitive-field classes, unknown fields, disguised identifiers, storage/condition, defect requirements and duplication, bounded notes, battery boundaries, activation-lock/ownership confirmation, price boundaries and coarse-region enforcement.
- **Green evidence:** [mobile-alpha-ci run 32735982788](https://github.com/gunflo1011-debug/asset-market-alpha/actions/runs/32735982788) passed client-secret scan, Expo dependency compatibility, strict TypeScript, all 20 intake tests, closed-alpha configuration and Android export. [Backend run 32735982656](https://github.com/gunflo1011-debug/asset-market-alpha/actions/runs/32735982656) also passed clean replay, pgTAP, lint and concurrency.
- **Implementation commits:** typed validator `6718f9f`; test suite `f7f734f`; isolated compile contract `42c67f6`; npm/CI wiring `8e31e12` and `9ca7617`. Final head `7b7cb5bbb6e0725d1971326f883238926197d24c` additionally carries the exact PR #3 scanner/telemetry fixes and PR #6 one-line smallint test cast because current main still fails those pre-existing gates; all identical overlaps are documented in PR #7 for clean rebase if PR #3/#6 land first.
- **Expected economic contribution:** converts the proposed Karlsruhe supply intake into an executable, low-support data contract while preventing collection of high-risk identifiers and precise location. This lowers privacy/incident/rework risk before demand validation; users, trades, revenue and profit remain UNKNOWN.
